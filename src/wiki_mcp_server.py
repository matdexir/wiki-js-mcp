...
async def wikijs_search_pages(query: str, space_id: str = None) -> str:
    ...
    try:
        ...
        NOTE: 
            ...
            - The following is a workaround when using WikiJS(V2) with postgres database while using postgres as the search engine.
            - Basically, the given ID here is not the true page ID but rather the search result ID given by the postgres vector operations.
            - We need to remediate by having an extra layer searching for the page's true ID.

            For more details, refer to: https://github.com/requarks/wiki/issues/2938
        """
        ...
        for item in search_data.get("results", []):
            ...
            for page in all_pages:
                if page.get("path") == item.get("path") and page.get(
                    "locale"
                ) == item.get("locale"):
                    page_id = page.get("id")
                    break
...
async def wikijs_repository_context() -> str:
    ...
    try:
        ...
        result = {
            ...
            if context
            else settings.DEFAULT_SPACE_NAME,
    ...
    try:
        ...
        result = {
            ...
            "hierarchy_summary": {
                ...
                "max_depth": max([p["path"].count("/") for p in pages_to_delete])
                if pages_to_delete
                else 0,
            },
            "status": "completed",
